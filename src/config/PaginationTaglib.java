...
if(offset==itr) 
{        
	if((itr/max+1)>10 && (itr/max+1)==(count/max))
	{ 
	out.write(constructLink(itr/max*steps, "...", "" , true)); 
	}
out.write(constructLink((itr/max+1)-1 *steps, String.valueOf(itr/max+1), "active", true));
}  
else if((itr/max+1)>10 && (itr/max+1)==(count/max))
{             
	out.write(constructLink(itr/max*steps, "...", "" , true)); 
	out.write(constructLink(itr/max*steps, String.valueOf(itr/max+1), "" , false)); 
}    
else if((itr/max+1)>10 && (itr/max+1)<(count/max))
{          
	out.write(constructLink(itr/max*steps, String.valueOf(itr/max+1), "hide_nav" , false)); 
}  
else    
{
